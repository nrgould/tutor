...
fn row_to_material(row: &Row) -> rusqlite::Result<CourseMaterial> {
    Ok(CourseMaterial {
        id: row.get(0)?,
        course_id: row.get(1)?,
        title: row.get(2)?,
...
#[tauri::command]
pub async fn create_course(input: CreateCourseInput) -> Result<Course, String> {
    let course_id = Uuid::new_v4().to_string();
    let now = chrono::Utc::now().format("%Y-%m-%d %H:%M:%S").to_string();
    let color = input.color.clone().unwrap_or_else(|| "#3b82f6".to_string());
    ...
    let course = Course {
        ...
        instructor: input.instructor.clone(),
...
#[tauri::command]
pub async fn get_courses(active_only: Option<bool>) -> Result<Vec<Course>, String> {
...
#[tauri::command]
pub async fn delete_course(course_id: String) -> Result<(), String> {
    with_connection(|conn| {
...
#[tauri::command]
pub async fn add_course_material(input: CreateMaterialInput) -> Result<CourseMaterial, String> {
...
pub async fn get_course_materials(
    ...
    with_connection(move |conn| {
        if let Some(ref mtype) = material_type {
            ...
            let materials = stmt
                .query_map([&course_id, mtype], row_to_material)?
                .collect::<Result<Vec<_>, _>>()?;
            Ok(materials)
        } else {
            let mut stmt = conn.prepare(
            ...
            let materials = stmt
                .query_map([&course_id], row_to_material)?
                .collect::<Result<Vec<_>, _>>()?;
            Ok(materials)
        }
    })
...
#[tauri::command]
pub async fn toggle_material_completed(material_id: String) -> Result<CourseMaterial, String> {
    with_connection(|conn| {
        conn.execute(
...
#[tauri::command]
pub async fn delete_material(material_id: String) -> Result<(), String> {
    with_connection(|conn| {
        conn.execute(
...
#[tauri::command]
pub async fn get_upcoming_assignments(days: Option<i32>) -> Result<Vec<CourseMaterial>, String> {
    ...
    with_connection(move |conn| {
        ...
        let materials = stmt
            .query_map([days], row_to_material)?
            .collect::<Result<Vec<_>, _>>()?;
