...
<div class="h-full flex flex-col bg-[var(--gray-1)]">
  ...
  <div class="flex items-center justify-between px-3 py-2 border-b border-[var(--gray-4)] bg-[var(--gray-2)]">
    <div class="flex items-center gap-1">
      ...
      <button
        class="p-2 rounded-lg transition-colors {isOverlayMode ? 'text-[var(--accent)] bg-[var(--accent-muted)]' : 'text-[var(--gray-9)] hover:text-[var(--gray-12)] hover:bg-[var(--gray-4)]'}"
        onclick={() => overlayModeStore.toggle()}
        title="{isOverlayMode ? 'Disable' : 'Enable'} overlay mode"
      >
        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
